...
bool check(ld x) {
    vector<ld> a1(n), p(n), m(n);
    REP(i, 0, n) a1[i] = a[i] - x;

    // Find prefix sum on this
    p[0] = a1[0];
    REP(i, 1, n) p[i] = p[i - 1] + a1[i];

    // Take min of pref sums
    m[0] = min(0.0l, p[0]);
    REP(i, 1, n) m[i] = min(m[i - 1], p[i]);

    // Check if any segment of length >= d has sum >= 0
    if (p[d - 1] >= 0) return true;
...
//    FILE_OUT
    ...
    // lo always possible, hi never possible
    ld lo = 0, hi = 101;
    REP(ii, 0, 100) {
        ld mid = (lo + hi) / 2;
        if (check(mid)) lo = mid;
