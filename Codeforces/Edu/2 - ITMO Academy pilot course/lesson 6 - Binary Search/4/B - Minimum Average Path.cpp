...
bool check(ld x) {
    vector<ld> c(n, LLONG_MAX);
    c[0] = 0;

    for (auto& [l, r, w] : a) {
        ld w1 = w - x;
        c[r - 1] = min(c[r - 1], c[l - 1] + w1);
    }
...
//    FILE_OUT
    ...
    cin >> n >> m;
    a.resize(m);
    REP(i, 0, m) cin >> get<0>(a[i]) >> get<1>(a[i]) >> get<2>(a[i]);
    sort(all(a));
    ...
    vi ans;
    int cur = n - 1;
    while (prev[cur] != 0) ans.push_back(cur = prev[cur]);
    reverse(all(ans));

    cout << ans.size() + 1 << endl;
    cout << "1 ";
    for (auto x1 : ans) cout << x1 + 1 << " ";
    cout << n << endl;
