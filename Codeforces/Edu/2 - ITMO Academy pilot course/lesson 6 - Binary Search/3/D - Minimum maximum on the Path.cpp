...
bool check(ll mid) {
    vl min_moves(n, INT_MAX);
    min_moves[0] = 0;

    for (auto& [u, v, c] : a) {
        if (c > mid) continue;

        min_moves[v] = min(min_moves[v], min_moves[u] + 1);
    }

    return min_moves[n - 1] <= d;
}
...
//    FILE_OUT
    ...
    // n cities, m roads, max d roads can use.
    cin >> n >> m >> d;
    a.resize(m);

    // Road connects a[i] to b[i] with cost c[i].
    REP(i, 0, m) {
    ...
    if (lo >= 1e9) {
        ...
        // Need to output k (the number of roads) and the movement path.
        // ans = hi.
        vl min_moves(n, INT_MAX), prev(n, -1);
        min_moves[0] = 0;

        for (auto& [u, v, c] : a) {
            if (c > hi) continue;

            if (min_moves[u] + 1 < min_moves[v]) {
                min_moves[v] = min_moves[u] + 1;
                prev[v] = u;
            }
        }

        cout << min_moves[n - 1] << endl;
        vi ans;
        n--;
        while (n != -1) {
            ans.push_back(n + 1);
