...
vi a;
stack<int> s1, s2;
stack<bitset<1005>> s1s, s2s;

bool good() {

    // cout << "good? " << endl;
    if (s1.empty() && s2.empty()) return false;
    if (s1.empty()) return s2s.top()[s];
    if (s2.empty()) return s1s.top()[s];

    // check if tops of s1s and s2s can make s.
    REP(i, 0, s) {
        if (s1s.top()[i] && s2s.top()[s - i]) {
            return true;
        }
    }
    return false;
