...
//    FILE_OUT
    ...
    // Segment good if sum of elements <= s.
    // Need to find sum of lengths of all good segments.
    // Invariant: if l..r is good, then l..j for all j <= r is good.
    // Find the first r for each l and then add lengths.
    ll ans = 0, cur = 0, j = 0;
    REP(i, 0, n) {
        // while remains good, push j
