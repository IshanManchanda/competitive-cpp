...
#define cinai(a, n) REP(iiii, 0, n) cin >> a[iiii]
...
//    FILE_OUT
    ...
    for (auto x : a) s += x;
    if (s * 3 >= p) {
        // Can be solved by triplicating a and then solving as usual.
        a.resize(3 * n);
        copy_n(a.begin(), n, a.begin() + n);
        copy_n(a.begin(), n, a.begin() + 2 * n);
        n1 = 3 * n;
    }
    else {
        // prefix + some_multiple_of_a + suffix.
        // multiple will be the maximum divided value.
        ll mul = p / s;
        ans += mul * n;
        p -= mul * s;

        // Now solve with double a
        a.resize(2 * n);
        copy_n(a.begin(), n, a.begin() + n);
        n1 = 2 * n;
    }

    ll cnt = INT_MAX, cur = 0, i_star = -1, j = 0;
    REP(i, 0, n1) {
        while (j < n1 && cur < p) {
            cur += a[j++];
        }
