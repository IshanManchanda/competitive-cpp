...
//    FILE_OUT
    ...
    // Good segment = GCD of all numbers = 1. Want minimum good segment.
    // It is easy to add element (take GCD of existing GCD and new number),
    // but expensive to remove. We can use 2-stack trick to implement easy
    // removal using easy insertion.
    // Same amortization trick with emulating a queue using 2 stacks.
    // We will maintain s1 and s2 -> s1 to dequeue and s2 to enqueue.
    // Along with s1 and s2, we store s1g and s2g which are running GCDs.
    stack<ll> s1, s2, s1g, s2g;
    ll ans = INT_MAX, j = 0;

    // General pattern for longest segment:
    // REP(j)
    //      push(j)
    //      while not good, pop(i)
    //      take ans

    // General pattern for shortest segment:
    // REP(i)
    //      while not good, push(j)
    //      if good: take ans
    //      pop(i)
    ...
    REP(i, 0, n) {
        ...
        while ((s1.empty() && s2.empty()) ||
               (s1.empty() && s2g.top() != 1) ||
               (s2.empty() && s1g.top() != 1) ||
               (!s1.empty() && !s2.empty() && gcd(s1g.top(), s2g.top()) != 1))
        {
            if (j == n) {
                check = 1;
                break;
            ...
            s2.push(a[j]);
            s2g.push((s2g.empty() ? a[j] : gcd(s2g.top(), a[j])));
            j++;
        }
        ...
        // remove i
        if (s1.empty()) {
            // move all from s2 to s1
            while (!s2.empty()) {
                s1.push(s2.top());
                s2.pop();
                s1g.push((s1g.empty() ? s1.top() : gcd(s1g.top(), s1.top())));
                s2g.pop();
            }
        }
        s1.pop();
