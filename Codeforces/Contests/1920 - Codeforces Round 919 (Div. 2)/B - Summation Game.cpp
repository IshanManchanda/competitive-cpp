...
#define NEW_PROBLEM
    ...
    TESTCASES {
        ...
        cinai(a, n);
        sort(a, a + n, greater<>());
        // maintain sliding window sum of next x elements, these will be -ve
        // and also sum of elements dropped
        // from these we can find best ans for alice
        int sum1 = accumulate(a, a + n, 0);
        int cur_sum = 0, sliding = accumulate(a, a + x, 0), ans;
        ans = sum1 - 2 * sliding;
//        cout << sum1 << " " << sliding << " " << ans << endl;
        REP(i, 0, k) {
            cur_sum += a[i];
            sliding -= a[i];
            if (i + x < n) sliding += a[i + x];
            ans = max(ans, sum1 - cur_sum - 2 * sliding);
        }
        cout << ans << endl;
