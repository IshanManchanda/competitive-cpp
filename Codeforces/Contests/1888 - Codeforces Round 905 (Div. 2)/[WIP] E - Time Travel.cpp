...
//    }
    int n, t;
    cin >> n >> t;
    unordered_map<int, set<int>> adj[t];
    set<int> imp[t];
    // array of size t of array of size n of vectors
    // array of size t of unordered_maps of vectors
    int m[t];
    REP(i, 0, t) {
        cin >> m[i];
        REP(j, 0, m[i]) {
            ...
            cin >> u >> v;
            u--, v--;
            adj[i][u].insert(v), adj[i][v].insert(u);
            imp[i].insert(u), imp[i].insert(v);
        }
    }
    int k;
    cin >> k;
...
//        cout << flush;
        ...
        for (auto it = cur.rbegin(); it != cur.rend(); it++) {
            auto [t1, u] = *it;
            if (t1 < last_a[a[i] - 1]) break;
            if (adj[a[i] - 1].find(u) == adj[a[i] - 1].end()) continue;
            for (auto v: adj[a[i] - 1][u]) {
                if (v == n - 1) {
                    cout << i + 1 << "\n" << flush;
                    return 0;
                }
                if (seen.find(v) == seen.end())
                    nxt.insert({i, v}), seen.insert(v);
            }
            adj[a[i] - 1].erase(u);
        }
        swap(nxt, cur);
...
//        for (auto u : imp[a[i] - 1]) {
//            if (cur.find(u) == cur.end()) continue;
//            if (adj[a[i] - 1].find(u) == adj[a[i] - 1].end()) continue;
//            for (auto v: adj[a[i] - 1][u]) {
//                if (v == n - 1) {
//                    cout << i + 1 << "\n" << flush;
...
//        }
        ...
        if (cur.size() * 2 < n) {
            ...
            for (int v = 0; v < n; v++) {
                if (cur.find(v) != cur.end()) continue;
                if (adj[a[i] - 1].find(v) == adj[a[i] - 1].end()) continue;
                for (auto u: adj[a[i] - 1][v]) {
                    if (cur.find(u) != cur.end()) {
                        nxt.insert(v);if (v == n - 1) {
                            cout << i + 1 << "\n" << flush;
                            return 0;
                        }
                    }
