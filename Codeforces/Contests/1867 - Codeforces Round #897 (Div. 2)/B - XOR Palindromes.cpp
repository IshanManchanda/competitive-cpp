...
//	cout << setprecision(11);
    ...
    TESTCASES {
        ...
        string s;
        cin >> s;
        // find number of diffs
        int diffs = 0;
        for (int i = 0; i < n / 2; i++) {
            diffs += s[i] != s[n - i - 1];
        }
//        cout << diffs << " " << n/2 - diffs << " " << n << endl;
        if (n % 2) {
            // odd length
            REP(i, 0, diffs) cout << "0";
            REP(i, 0, 2 * (n/2 - diffs + 1)) cout << "1";
            REP(i, 0, n + 1 - 2 * (n/2 - diffs + 1) - diffs) cout << "0";
        }
        else {
            // even length
            REP(i, 0, diffs) cout << "0";
            cout << "1";
            REP(i, 0, n/2 - diffs) cout << "01";
            REP(i, 0, n - 2 * (n/2 - diffs) - diffs) cout << "0";
        }
        cout << "\n";
    }
