...
int dfs(int x, int p) {
    vis[x] = p;
    // need to update to a[x]. check that position
    if (vis[a[x] - 1] == -1) return vis[x] = -1;
    if (vis[a[x] - 1]) {
        // we've visited
        // if length of k then gg, else L
        if (p - vis[a[x] - 1] == k - 1) return vis[x] = -1;
        else return vis[x] = -100;
    }
    vis[x] = p;
    return vis[x] = dfs(a[x] - 1, p + 1);
}
