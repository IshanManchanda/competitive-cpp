...
//        cout << freq << endl;
        ...
        unordered_map<int, unordered_set<int>> excl;
        unordered_map<int, unordered_set<int>> multss, divs;
        REP(i, 1, maxa + 1) {
            if (freq[i] == 0) continue;
            // for this number, find all multiples of it present in the arr
            // and to their sets, add all others
//            set<int> mults;
            for (ll j = i; j <= maxa; j += i) if (freq[j]) multss[i].insert(j);
            for (auto j : multss[i]) divs[j].insert(i);
//            for (auto j : mults) {
//                excl[j].insert(mults.begin(), mults.end());
//            }
        }
        ...
        REP(i, 1, maxa + 1) {
            ...
            for (auto x1 : divs[i]) mults.insert(all(multss[x1]));
//            cout << excl[i] << endl;
            for (auto x : mults) ans += freq[i] * 1ll * freq[x];
        }
        cout << (n * n - ans) / 2 << endl;
    }
