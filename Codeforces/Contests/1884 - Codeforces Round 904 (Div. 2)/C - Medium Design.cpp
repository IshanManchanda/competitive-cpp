...
//	cout << setprecision(11);
    ...
    TESTCASES {
        ...
        REP(i, 0, n) {
            ...
            open.push_back(l);
            close.push_back(r);
            if (l == 1) open0.push_back(r);
            if (r == m) closeN.push_back(l);
...
//        cout << pts << endl;
        int ans = 0;
        for (auto x : pts) {
            // find number that close before x, open after x
            int bef = lower_bound(all(close), x) - close.begin();
            int aft = n - (upper_bound(all(open), x) - open.begin());
            int sect = n - bef - aft;
            // find how many of these intersect 0 and how many N
            int int0 = open0.end() - lower_bound(all(open0), x);
            int intN = upper_bound(all(closeN), x) - closeN.begin();
//            cout << x << ":: ";
//            cout << bef << " " << aft << ": " << sect << ". " << int0 << ", " << intN << endl;
            ans = max(ans, sect - min(int0, intN));
        }
        cout << ans << endl;
    }
