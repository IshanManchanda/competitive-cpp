...
#define NEW_PROBLEM
    ...
    TESTCASES {
        ...
        // number of pairs such that AND is greater or equal to XOR
        // Consider an index where both 0, both AND and XOR same
        // Consider an index where both 1, AND will be greater than XOR
        // Consider an index where one is 1. XOR will be greater
        // The first set bit of this number should be the first set bit of the other number
        // floor(log2) gives us that
        vi cnt(64);
        ll ans = 0;
        for (auto x : a) {
            int k = log2(x);
