...
void dfs(int i, int j, int r, int c, vvi &board) {
    cnt++;
    board[i][j] = 2;
    int dx[] = {1, 0, -1, 0};
    int dy[] = {0, 1, 0, -1};
    for (int k = 0; k < 4; k++) {
        ...
        if (x >= 0 && y >= 0 && x < r && y < c) {
            ...
            if (board[x][y] == 1) dfs(x, y, r, c, board);
            else if (board[x][y] == 0) pts.insert({x, y});
        }
    }
}
    ...
    TESTCASES1 {
        ...
        REP(i, 0, r) {
            REP(j, 0, c) {
                ...
                dfs(i, j, r, c, board);
                if (pts.size() == 1) {
                    // add to this point the size
                    score[*pts.begin()] += cnt;
                    ans = max(ans, score[*pts.begin()]);
                }
//                cout << "DFS from " << i << " " << j << ": ";
//                cout << cnt << ". " << pts << endl;
            }
