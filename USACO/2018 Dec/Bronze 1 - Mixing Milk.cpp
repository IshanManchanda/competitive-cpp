...
//    TESTCASES {}
    int cur[3], cap[3];
    REP(i, 0, 3) cin >> cap[i] >> cur[i];
    REP(i, 0, 100) {
        int source = i % 3, dest = (i + 1) % 3;
        // pour from source to dest
        int pour = min(cur[source], cap[dest] - cur[dest]);
        cur[dest] += pour, cur[source] -= pour;
    }
    for (auto x : cur) cout << x << endl;
    cout << flush;
}
