...
//    TESTCASES {}
    int n;
    cin >> n;
    // char count each word in the pair,
    // then take element-wise max.
    // add these to ans.
    vi ans(26, 0), freq1, freq2;
    REP(i, 0, n) {
        freq1.assign(26, 0), freq2.assign(26, 0);
        string s1, s2;
        cin >> s1 >> s2;
        for (auto x : s1) freq1[x - 'a']++;
        for (auto x : s2) freq2[x - 'a']++;
        REP(j, 0, 26) ans[j] += max(freq1[j], freq2[j]);
    }
