...
//	cout << setprecision(11);
    string problem_name = "measurement";
#ifndef LOCAL_RIPPR
...
//    TESTCASES {}
    int n;
    cin >> n;
    map<string, int> mapper = {{"Mildred", 0}, {"Elsie", 1}, {"Bessie", 2}};
    vi milk(3, 7);
    int cur = 1 | 2 | 4, ans = 0;
    vvi inp(n, vector<int>(3, 0));
    REP(i, 0, n) {
        string s;
        cin >> inp[i][0] >> s >> inp[i][2];
        inp[i][1] = mapper[s];
    }
    sort(inp.begin(), inp.end());
//    for (auto x : inp) cout << x << endl;
    REP(i, 0, n) {
        // update cow
        milk[inp[i][1]] += inp[i][2];

        // find max
        int max_milk = max({milk[0], milk[1], milk[2]});
        int max_display = 0;
        REP(j, 0, 3)
            if (milk[j] == max_milk)
                max_display |= bin_exp_mod(2, j, 1e9 + 7);
