...
//    cout << setprecision(11);
    ...
    TESTCASES {
        ...
        vi b(n);
        for (int i = 0; i < n; i++) {
            // a[i] is the person sitting in position i. Then the position
            // of the a[i]'th person is:  i
            b[a[i] - 1] = i + 1;
        }
        a = b;
//        cout << a << endl;
        ...
        pos.insert(n);
        dists.insert(n - 2);
        // X _ X = 3 - 1 = 2 / 2 = 1 - 1 = 0 gap.
        // X _ _ X = 4 - 1 = 3 / 2 = 1 - 1 = 0 gap.
        // X _ _ _ X = 5 - 1 = 4 / 2 = 2 - 1 = 1 gap.
        int ans = 1;
        for (int i = 2; i < n; i++) {
            // only need to check till (n + 1) / 2
//            cout << i << endl;
            if (i >= (n + 1) / 2) {
                break;
            }
            // find the smallest number in the set that is greater than a[i]
            auto it = pos.lower_bound(a[i]);
            // find the greatest number in the set that is less than a[i]
            auto it2 = pos.lower_bound(a[i]);
            it2--;

            // check gaps between positions and a[i].
            // find the max element in dists
            ll gap = (*dists.rbegin() - 1) / 2;
            ll d1 = *it - a[i] - 1;
            ll d2 = a[i] - *it2 - 1;
//            cout << i << ": " << a[i] << endl;
//            cout << pos << endl;
