...
//    cout << setprecision(11);
    ...
    TESTCASES {
        ...
        if (m == 1) {
            ...
            // find index of minimum of b
            ll min_idx = 0;
            for (ll i = 1; i < m; i++) {
                if (b[i] < b[min_idx]) min_idx = i;
            }
            // create array c which is rotation of b beginning at this index.
            vi c(m);
            for (ll i = 0; i < m; i++) {
                c[i] = b[(i + min_idx) % m];
            }
...
//                cout << i << ": " << ans << endl;
                if (i == n - m) {
                    if (replaced) {
                        for (int j = 0; j < m; j++) ans.push_back(c[j]);
                        break;
                    }
                    if (a[i] < c[0]) {
...
//                    cout << a_smaller << endl;
    ...
    // if we replace at any place, we are forced to always replace after that place.
