...
//	cout << setprecision(11);
    ...
    TESTCASES {
        ...
        if (k == 0) {
            ...
            ll ways = 1;
            REP(i, 1, max(o, e) + 1) ways = (ways * i) % MOD;
            cout << ways << endl;
            continue;
        }
        // k is 1, need to have equal or at most differ by 1
        if (abs(o - e) > 1) {
            cout << "0\n";
            continue;
        }
        if (n % 2 == 0) {
            // equal number, can start on either
            ll ways = 2;
            REP(i, 1, e + 1) ways = (ways * i) % MOD;
            REP(i, 1, o + 1) ways = (ways * i) % MOD;
            cout << ways << endl;
            continue;
        }
        // odd number, have to start at the greater
        ll ways = 1;
        REP(i, 1, e + 1) ways = (ways * i) % MOD;
        REP(i, 1, o + 1) ways = (ways * i) % MOD;
        cout << ways << endl;
