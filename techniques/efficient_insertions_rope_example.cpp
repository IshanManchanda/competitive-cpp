...
//    TESTCASES {}
    // Problem: Given array of size n <= 1e5, q <= 1e5 queries
    // query 1 L: Move L places to the left
    // query 2 R: Move R places to the right
    // query 3 X Y: Remove the next number in the sequence and insert X and Y
    // (number of elements increases by 1)

    // In one variant of the problem, L, R <= 5.
    // In this case, we can simply use a linked list as only insertions
    // need to be efficient.
    // Without that constraint, we need random access (move pointer around)
    // and also efficient insertions.
    // Some solutions: Rope, Order Statistics Set, Skip List, Sqrt Decomp

    // Rope: High constant factors but lg n insertions with random access
    ...
    while (q--) {
        ...
        if (t == 1 || t == 2) {
            ...
            int x, y;
            cin >> x >> y;
            v.mutable_reference_at(c + 1) = x;
            v.insert(v.mutable_begin() + c + 2, y);
        }
    }
    for (auto x : v) cout << x << " ";
    cout << flush;
