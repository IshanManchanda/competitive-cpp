...
//    }
    int n, m;
    cin >> n >> m;
    vvi adj[n]; // adj list will have {B, X, Y}
    REP(i, 0, m) {
        int a1, b1, x1, y1;
        cin >> a1 >> b1 >> x1 >> y1;
        a1--, b1--;
        adj[a1].push_back({b1, x1, y1});
        adj[b1].push_back({a1, -x1, -y1});
    }
    vl xi(n, LLONG_MAX), yi(n, LLONG_MAX);
    xi[0] = yi[0] = 0;
    queue<int> q;
    q.push(0);
    while (!q.empty()) {
    ...
    REP(i, 0, n) {
        if (xi[i] == LLONG_MAX) cout << "undecidable\n";
        else cout << xi[i] << " " << yi[i] << "\n";
    }
