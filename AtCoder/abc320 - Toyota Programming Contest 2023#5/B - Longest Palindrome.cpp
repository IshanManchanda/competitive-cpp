...
//    }
    ...
    int n = s.size();
    int dp[n][n];
    // is there a palindrome starting at i ending at i + j (incl)?
    memset(dp, 0, sizeof dp);
    REP(i, 0, n) dp[i][0] = 1;
    int ans = 1;
    REP(i, 0, n - 1) ans = max(ans, dp[i][1] = 2 * (s[i] == s[i + 1]));
    REP(j, 2, n) {
        REP(i, 0, n - j) {
            if (s[i] == s[i + j] && dp[i + 1][j - 2]) dp[i][j] = max(dp[i][j], 2 + dp[i + 1][j - 2]);
            ans = max(ans, dp[i][j]);
        }
    }
//    REP(i, 0, n) {
