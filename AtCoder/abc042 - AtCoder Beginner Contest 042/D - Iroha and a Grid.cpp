...
//    }
    ...
    // precomp factorials and inverses under mod
    vl fact(h + w, 1), inv(h + w, 1);
    REP(i, 1, h + w) {
    ...
    REP(j, b, w) {
        ll n1 = h - a - 1 + j;
        ll r1 = j;
        ll term = (((fact[n1] * inv[r1]) % mod) * inv[n1 - r1]) % mod;
        ll n2 = a - 1 + w - j - 1;
        ll r2 = a - 1;
        term = (((((term * fact[n2]) % mod) * inv[r2]) % mod) * inv[n2 - r2]) % mod;
        ans = (ans + term) % mod;
    }
    cout << ans % mod;
