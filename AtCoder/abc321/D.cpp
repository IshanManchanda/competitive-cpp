...
//    }
    ...
    ll ans = 0;
    // each item in a taken m times. of these, find how many actual
    // + how many p
    // also for b, use a prefix sum
    ll pref[m + 1];
    pref[0] = 0;
    REP(i, 0, m) pref[i + 1] = pref[i] + b[i];
    for (auto x : a) {
        // find all the elements such that a + b < p
        // lower_bound -> first element >=
        ll c = lower_bound(b, b + m, p - x) - b;
        ans += x * c + pref[c];
        ans += (m - c) * p;
    }
    cout << ans << endl;
