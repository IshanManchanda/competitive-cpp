...
//    cout << ps << endl;
    ...
    for (ll x : ps) {
        bool x_odd = x % 2;
        ans_even |= (b_odd & x_odd);
        d *= (1 + (b * x) % mod) % mod;
        d %= mod;
    }
    // need to track if ans is even or odd
    // if we mult by at least 1 even number, it should be even?
    // we mult by 1 + b * x
    // so if both b and x are odd
//    cout << ans_even << endl;
//    cout << d << endl;

    ll ans = (b * d) % mod;
    if (!ans_even) ans = (ans - 1) % mod;
    ans *= mod_inv(2, mod);
    cout << ans % mod;
