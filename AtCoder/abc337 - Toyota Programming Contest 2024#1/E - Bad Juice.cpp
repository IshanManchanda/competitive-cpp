...
#define NEW_PROBLEM
    ...
    // 2^n can be tested, take ceil(log2)
    int n;
    cin >> n;
    int m = ceil(log2(n));
    cout << m << "\n" << flush;
    vvi serve(m, vi());
    // for each of n bottles, we serve it to those which have 1 in the bit position
    REP(i, 1, n + 1) {
        REP(j, 0, m) {
            if (i & (1 << j)) serve[j].PB(i);
        }
    }

    REP(i, 0, m) {
        cout << serve[i].size();
        for (auto x : serve[i]) cout << " " << x;
        cout << endl;
    }
    cout << flush;
    ...
    cin >> s;
    reverse(all(s));
    // bits of s tell which prisoners died. We need to find
    // the bottle which were fed to these
    // a bottle was fed to x if
