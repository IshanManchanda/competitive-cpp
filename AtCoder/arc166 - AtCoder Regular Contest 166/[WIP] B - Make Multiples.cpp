...
//	cout << setprecision(11);
    ...
    TESTCASES {
        ...
        ll x[n];
        cinai(x, n);
        // for each, find dists to ma, mb, mc, mab, mbc, mca, mabc
        ll ab = a * b / __gcd(a, b);
        ll bc = b * c / __gcd(b, c);
        ll ca = c * a / __gcd(c, a);
        ll abc = ab * c / __gcd(ab, c);
        ll ma[n], mb[n], mc[n], mab[n], mbc[n], mca[n], mabc[n];
        ll ans = LLONG_MAX;

        REP(i, 0, n) {
            // if rem != 0, add n - rem
            ma[i] = x[i] % a ? a - (x[i] % a) : 0;
            mb[i] = x[i] % b ? b - (x[i] % b) : 0;
            mc[i] = x[i] % c ? c - (x[i] % c) : 0;

            mab[i] = x[i] % ab ? ab - (x[i] % ab) : 0;
            mbc[i] = x[i] % bc ? bc - (x[i] % bc) : 0;
            mca[i] = x[i] % ca ? ca - (x[i] % ca) : 0;

            mabc[i] = x[i] % abc ? abc - (x[i] % abc) : 0;
            ans = min(ans, mabc[i]);
        }
        // consider top 3 for each - store set of pairs?
    }
    cout << flush;
}
