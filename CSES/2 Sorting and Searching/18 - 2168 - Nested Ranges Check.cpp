...
//    TESTCASES {}
    ...
    while (i < n) {
        ...
        while (j < n && ranges[j][0] == ranges[i][0]) {
            // check if any closing before or eq current's
            // can it from start, it should be first or second
            bool flag = false;
            for (auto it = closing.begin(); it != closing.end() && it->ff <= ranges[j][1]; it++) {
                if (it->ff < ranges[j][1] || it->ss != ranges[j][2]) {
                    flag = true;
                    break;
                }
            }
            if (flag) ans1[ranges[j][2]] = 1;
            j++;
        }
    ...
    while (i > -1) {
        ...
        while (j > -1 && ranges[j][0] == ranges[i][0]) {
            bool flag = false;
            for (auto it = closing2.rbegin(); it != closing2.rend() && it->ff >= ranges[j][1]; it++) {
                if (it->ff > ranges[j][1] || it->ss != ranges[j][2]) {
                    flag = true;
                    break;
                }
            }
            if (flag) ans2[ranges[j][2]] = 1;
            j--;
        }
