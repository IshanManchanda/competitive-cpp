...
ll solve(int i, int j, vi &a) {
    if (i > j) return 0;
    if (i == j) return a[i];
    if (dp[i][j] != -1) return dp[i][j];

    // take first or second
    return dp[i][j] = max(a[i] - solve(i + 1, j, a), a[j] - solve(i, j - 1, a));
}
...
//    TESTCASES { }
    ...
    cinai(a, n);
    dp.assign(n, vl(n, -1));
    // solve(i, j, a) -> max possible score
    ll diff = solve(0, n - 1, a), s = 0;
    for (auto x : a) s += x;
    cout << (diff + s) / 2;
